...
"
  ...
  (let (time-buffer
    ...
    (cl-flet*
           ...
           (with-temp-buffer
             ...
             (if
                 (catch 'cannot-parse-date-time
                   (while
                       (and (not (eobp))
                            (not (with-current-buffer time-buffer
                                   (eobp))))
                     (cond
                      ;; ((handle-item) t)
                      (t
      ...
      (setq format (replace-regexp-in-string format "%e" "%d"))
      (setq format (replace-regexp-in-string format "%A" "%a"))
      (setq format (replace-regexp-in-string format "%B" "%b"))
      (setq format (replace-regexp-in-string format "%h" "%b"))
      ;; Break down composite cases to individual components.
      (setq format (replace-regexp-in-string format "%D" "%m/%d/%y"))
      (setq format (replace-regexp-in-string format "%T" "%H:%M:%S"))
      (setq format (replace-regexp-in-string format "%R" "%H:%M"))
