...
(defun ej/switch-shell-cycle (&optional inc)
  "Switch to next/prev shell buffer.
  INC can be -1 (prev) or 1 (next). Defaults to 1 (next)."
  (interactive)
  (let* ((inc (or inc 1))
         (shell-buffers (ej/get-shell-buffers)))
    (if (not shell-buffers) (message "No *shell*<N> buffers found!")
      (let* ((sorted-shell-buffers (-sort (-on #'< #'extract-int) shell-buffers))
             (len (length sorted-shell-buffers))
             (cur-buf (buffer-name))
             (idx (-elem-index cur-buf sorted-shell-buffers))
             (next-idx (if idx (mod (+ idx inc) len) 0))
             (next-buf (elt sorted-shell-buffers next-idx)))
        (message "Going to switch to buffer: %s" next-buf)
...
(global-set-key (kbd "M-s-n") (lambda () (interactive) (ej/switch-shell-cycle -1)))
(global-set-key (kbd "M-s-m") 'ej/switch-shell-cycle)
