...
(global-set-key (kbd "C-x s-e") 'ej/replace-last-sexp)
(global-set-key (kbd "s-h") 'ej/copy-all)
...
(global-set-key (kbd "C-x C-S-e") 'ej/replace-last-sexp)
...
(defun ej/calculator ()
  ...
  (let* ((p (point))
         ...
         (indexof (search SEARCH-START-CHARACTER almost-expr :start2 0))
...
(defun ej/time (&optional mode)
  (let* ((time-entry (assoc mode TIME-FORMATS))
        (time-fmt (if (null time-entry) TIME-FORMAT-DEFAULT (cadr time-entry))))
    (format-time-string time-fmt (current-time))))
...
(defun ej/punto (beg end &optional arg)
  ...
  (do () ((>= (point) end))
    (setq translated-char (ej/punto-translate-char (char-after (point))))
...
(defhydra ej/hydra-buffers-windows (:foreign-keys warn :columns 1)
  ...
  ("j" (progn (setq *run-again* t) (ej/navigate-main 1)) "jump" :color blue)
...
#+end_src
** hydra: interactive emacs lisp
#+begin_src emacs-lisp
(defun ej/replace-last-sexp (mode)
  (interactive "p")
...
(defun ej/cur-sexp ()
  ...
  (let* ((pos-end (point))
         (_ (backward-sexp)))
    (read (buffer-substring-no-properties (point) pos-end))))
...
(defun ej/setq-let ()
  ...
  (save-excursion
    (let* ((sexp (ej/cur-sexp))
           (var (car sexp)))
			(eval `(setq ,var ,(cadr sexp)))
			(message "%s >> %s" var (symbol-value var)))))

(global-set-key (kbd "C-x C-M-e") 'ej/setq-let)
(global-set-key (kbd "C-x C-s-e") 'ej/setq-let)

(defun ej/setq-last-sexp (var)
  (interactive "senter var name: ")
  (save-excursion
    (eval `(setq ,(read var) ,(ej/cur-sexp)))
...
(defhydra ej/elisp-interactive (:columns 1 :foreign-keys warn :exit t)
  "Emacs Lisp interactive stuff"
  ("r" ej/replace-last-sexp "replace last sexp")
  ("s" ej/setq-last-sexp "setq last sexp")
  ("m" ej/message-last-sexp "message last sexp")
  ("l" ej/setq-let "setq last let-sexp")
  ("k" ej/setq-killed "setq killed")
  ("<ESC>" nil "exit")
  )
(global-set-key (kbd "<M-f5>") 'ej/elisp-interactive/body)
#+end_src
** hydra: yank patchers
#+begin_src emacs-lisp
...
(defun ej/yank-encode-wiki-links ()
  ...
  (let* ((killed (substring-no-properties (current-kill 0)))
         ...
         (patched-killed (if is-link (decode-coding-string (url-unhex-string killed) 'utf-8) killed))
         (patched-killed-2 (s-replace " " "%20" patched-killed)))
    (insert patched-killed-2)))

(defun ej/yank-title-as-filename ()
  (interactive)
  (let* (
         (killed (substring-no-properties (current-kill 0)))
         (patched (s-replace-regexp "[\s\n_–]" "-" (downcase killed)))
         (patched2 (s-replace "," "" patched))
         (patched3 (s-replace ":" "" patched2))
         )
    (insert patched3)))
...
(defun ej/yank-link-to-wiki ()
  (interactive)
  (let* (
         (link (substring-no-properties (current-kill 0)))
         (title-raw (cadr (split-string link "/wiki/")))
         (title-raw2 (decode-coding-string (url-unhex-string title-raw) 'utf-8))
         (title (s-replace "_" " " title-raw2))
         )
    (insert (format "[[%s][wiki: %s]]" link title))))

(defun ej/yank-python-onelinefy ()
  (interactive)
  (let* ((code (substring-no-properties (current-kill 0)))
         (code-2 (s-replace-regexp "\n *" "; " code))
         (code-3 (s-replace ":;" ":" code-2))
         (code-4 (s-trim code-3)))
     (insert (format "`%s`" code-4))))
...
(global-set-key (kbd "<f11>") 'org-latex-export-to-pdf)
(defun ej/toggle-bind-f11-export-to-beamer ()
  " TODO: if #+BEAMER_THEME in document, export to beamer "
  (interactive)
  (let ((fun (if (eq 'org-latex-export-to-pdf (key-binding (kbd "<f11>")))
               'org-beamer-export-to-pdf
               org-latex-export-to-pdf)))
    (message "switching export to %s." fun)
    (global-set-key (kbd "<f11>") 'org-beamer-export-to-pdf)))
#+end_src
** xelatex
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp :tangle no
(setq TIME-FORMATS-2
      (cl-flet ((patch-16 (e) (if (eq (car e) 16) `("w" ,(cadr e)) e)))
        (--map (patch-16 it) TIME-FORMATS)))

(defun ej/ins-time ()
  (interactive)
  (let ((sexp (mapcar (lambda (f)
                        (let ((key (car f)) (time (format-time-string (cadr f))))
                          (list (format "\t\t%s" key) `(insert ,time) time))) TIME-FORMATS-2)))
    (call-interactively (eval
                         `(defhydra hydra-ins-time (:exit t :columns 1 :foreign-keys warn)
                            "Hydra insert time"
                            ,@sexp
                            ("\t\tq" nil "quit"))))))
