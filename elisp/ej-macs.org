...
(global-set-key (kbd "M-s-n") 'ej/switch-next-shell)
#+end_src
*** local shortcuts
#+begin_src emacs-lisp
...
(define-key shell-mode-map (kbd "C-c s-r") #'rename-shell)
#+end_src
...
(defun ej/annotate-py-with-flake8 ()
  (interactive)
  (save-buffer)
...
(pretty-hydra-define ej/python-interactive (:foreign-keys warn :exit t :quit-key "q")
   ...
   "Annotations"
    ...
    ("b" (insert "breakpoint()") "breakpoint")
    ("a" (insert "from pathlib import Path") "from pathlib import Path")
    ("m" (insert "if __name__ == '__main__':\n    ") "if __name__ == '__main__'")
		)
...
Installing =littler= required (=sudo apt install littler=). If no littler found, uses =awk=.
...
#+end_src
** quick copy
I often had to copy =shrugman=, so I wrote function for it.
Then I realized that I needed more different copy options.

- [ ] extract transient+helm part from nav-nav, use it here
#+begin_src emacs-lisp
...
(defun ej/is-python-in-command ()
  (save-excursion
    (move-beginning-of-line 1)
    (let* ((cmd (thing-at-point 'line))
           (py-words '("python" "pytest" "PYTHONPATH" "flake8"))
           (res (and cmd (--any? (s-contains? it cmd) py-words)))
           ) res)))
...
(defun ej/run-other-window ()
  ...
  (let* ((is-py (s-ends-with? ".py" (buffer-file-name))))
    ...
    (if (not (equal major-mode 'shell-mode))
      ...
      (goto-char (point-max))
      (when (ej/is-inside-program)
        (comint-send-eof))
      (if (not is-py) (comint-previous-input 1)
        (while (not (ej/is-python-in-command))
...
(defun ej/wrap-src (lang)
  (let* ((snippet (buffer-substring-no-properties (mark) (point)))
         (pref-base (if (equal major-mode 'org-mode) ORG-BASE-PREF MD-BASE-PREF))
         (suff-base (if (equal major-mode 'org-mode) ORG-BASE-SUFF MD-BASE-SUFF))
         (pref (format "%s%s\n" pref-base lang))
         (suff (format "%s%s\n" suff-base (if (zerop (current-column)) "" "\n")))
         (block (format "%s%s%s" pref snippet suff)))
