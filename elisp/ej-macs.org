...
(defun ej/defun-assign ()
  (interactive)
  (let* ((sexp (ej/cur-sexp))
         (_ (when (not (equal (car sexp) 'defun)) (error nil)))
         (name (cadr sexp))
         (args (caddr sexp))
         (args/ (--map (intern (concat (symbol-name it) "/")) args))
         (assignments (-flatten-n 1 (--map (list (car it) (cdr it)) (-zip args args/)))))
    (ej/replace-last-sexp `(defun ,name (,@args/) (setq ,@assignments)))))

(defun ej/s-prefix (str drop-last)
  (substring str 0 (- (length str) drop-last)))

(defun ej/defun-deassign ()
  (interactive)
  (let* ((sexp (ej/cur-sexp))
         (_ (when (not (equal (car sexp) 'defun)) (error nil)))
         (name (cadr sexp))
         (args/ (caddr sexp))
         (args (--map (intern (ej/s-prefix (symbol-name it) 1)) args/)))
    (ej/replace-last-sexp `(defun ,name (,@args)))))
...
(defhydra ej/elisp-interactive (:columns 1 :foreign-keys warn :exit t)
  ...
  ("e" eros-eval-last-sexp "eval last sexp" :exit t)
  ("o" eval-buffer "eval-buffer")
  ("p" (ert t) "ert")
  ("a" ej/defun-assign "make setq-assigner from defun")
  ("A" ej/defun-deassign "unmake setq-assigner from defun")
  ("T" ej/toggle-el-test "toggle .el or -test.el")
  ("<ESC>" nil "exit")
  ("C-n" (next-line) "exit" :exit t)
  )
