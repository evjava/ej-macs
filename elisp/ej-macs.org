...
(defun ej/is-python-in-command ()
  (save-excursion
    ...
    (let* ((word (thing-at-point 'word))
           (py-words '("python" "pytest" "PYTHONPATH")))
      (--any? (equal it word) py-words))))
...
(defun ej/wrap-src (lang)
  (let* ((snippet (buffer-substring-no-properties (mark) (point)))
         (prefix (format "#+begin_src %s\n" lang))
         (suffix (format "%s#+end_src\n" (if (zerop (current-column)) "" "\n")))
         (block (format "%s%s%s" prefix snippet suffix)))
...
(defun ej/wrap-src-interactive ()
  (interactive)
  (let* ((snippet (buffer-substring-no-properties (mark) (point)))
         (detected-lang-0 (symbol-name (language-detection-string snippet)))
         (detected-lang (if (equal "awk" detected-lang-0) "emacs-lisp" detected-lang-0))
